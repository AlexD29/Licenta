...
import EmotionPieChart from 'charts/ArticleDetails/EmotionPieChart';

const PoliticianTopEntities = ({ politicianId }) => {
    ...
    useEffect(() => {
        const fetchTopEntities = async () => {
            try {
                const response = await fetch(`http://localhost:8000/api/politician-top-entities?politician_id=${politicianId}`);
                if (!response.ok) {
                    throw new Error('Network response was not ok.');
                }
                const data = await response.json();
                setTopEntities(data.top_entities);
                setEntityName(data.politician_name);
                setLoading(false);
            } catch (error) {
...
export default PoliticianTopEntities;

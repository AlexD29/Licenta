...
const PoliticianArticlesChart = ({ politicianId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/politician-articles-distribution-since-january/${politicianId}`);

        const { dates, positive_counts, negative_counts, neutral_counts, total_counts, politician_name } = response.data;

        setPoliticianName(politician_name);
...
export default PoliticianArticlesChart;

...
const TopPoliticianAuthorsPieChart = ({ politicianId }) => {
  const [chartData, setChartData] = useState([]);
  const [name, setName] = useState([]);

  useEffect(() => {
    const fetchTopPoliticianAuthorsData = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/top-politician-authors', {
          params: { politician_id: politicianId }
        });
        const data = response.data.top_authors_data;
        const name = response.data.politician_name;
        setName(name);
        ...
        console.error('Error fetching top politician authors data:', error);
      }
    };

    fetchTopPoliticianAuthorsData();
  }, [politicianId]);
  ...
  const option = {
    ...
    series: [
      {
        name: 'Top Authors for Politician',
        type: 'pie',
        radius: ['40%', '70%'],
...
export default TopPoliticianAuthorsPieChart;

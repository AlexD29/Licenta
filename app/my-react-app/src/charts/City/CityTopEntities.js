...
const CityTopEntities = ({ cityId }) => {
    ...
    useEffect(() => {
        const fetchTopEntities = async () => {
            try {
                const response = await fetch(`http://localhost:8000/api/city-top-entities?city_id=${cityId}`);
                if (!response.ok) {
                    throw new Error('Network response was not ok.');
                }
                const data = await response.json();
                setTopEntities(data.top_entities);
                setEntityName(data.city_name);
                setLoading(false);
            } catch (error) {
...
export default CityTopEntities;

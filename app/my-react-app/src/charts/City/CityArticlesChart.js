...
const CityArticlesChart = ({ cityId }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`http://localhost:8000/api/city-articles-distribution-since-january/${cityId}`);

        const { dates, positive_counts, negative_counts, neutral_counts, total_counts, city_name } = response.data;
...
export default CityArticlesChart;

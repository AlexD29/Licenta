...
const TopPoliticalPartyAuthorsPieChart = ({ politicalPartyId }) => {
  const [chartData, setChartData] = useState([]);
  const [name, setName] = useState([]);

  useEffect(() => {
    const fetchTopPoliticalPartyAuthorsData = async () => {
      try {
        const response = await axios.get('http://localhost:8000/api/top-political-party-authors', {
          params: { political_party_id: politicalPartyId }
        });
        const data = response.data.top_authors_data;
        const name = response.data.political_party_name;
        setName(name);
        ...
        console.error('Error fetching top political party authors data:', error);
      }
    };

    fetchTopPoliticalPartyAuthorsData();
  }, [politicalPartyId]);
  ...
  const option = {
    ...
    series: [
      {
        name: 'Top Authors for Political Party',
        type: 'pie',
        radius: ['40%', '70%'],
...
export default TopPoliticalPartyAuthorsPieChart;

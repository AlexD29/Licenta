...
const PoliticianPage = ({ userId }) => {
    ...
    const fetchPolitician = async () => {
      try {
        const response = await axios.get(
          `http://localhost:8000/api/politician/${id}`,
          {
  ...
  const handleAddFavorite = async (politician) => {
    try {
      if (!userId) {
      ...
      const action = politician.isFavorite ? "remove" : "add";

      await axios.post("http://localhost:8000/api/favorites", {
        user_id: userId,
        entity_id: politician.id,
        entity_type: politician.entity_type,
        action: action,
      });
      setPolitician({ ...politician, isFavorite: !politician.isFavorite });
    } catch (error) {
  ...
  const charts = useMemo(() => {
    ...
    return [
      ...
      <PoliticianPartyArticlesCount
        politicianId={id}
        politicalPartyId={politician.political_party_id}
      />,
  ...
  const shuffledCharts = useMemo(() => {
    ...
    return {
      firstPart: shuffled.slice(0, 5),
      thirdPart: shuffled.slice(5, 10)
    };
  }, [charts]);
    ...
    <div className="container">
      <div className="entity-page-first-part">
        <div className="entity-first-part-centered">
          <div className="background-div politician"></div>
          <div className="profile-info">
            <div className="info-first-column">
              <img
                src={politician.image_url}
                alt={`${politician.first_name} ${politician.last_name}`}
                className="entity-profile-picture politician-picture"
              />
...
c1.522-1.906,2.189-4.145,1.991-6.716C460.068,229.007,459.021,226.961,457.122,225.438z"
        ...
        <div className="other-details-box">
          ...
          <span>{formatDate(politician.date_of_birth)}</span>
          <span>{calculateAge(politician.date_of_birth)} de ani</span>
        </div>
        ...
        <div className="social-media-icons-entity">
          <a
          className="social-media-link"
            href={politician.facebook_link}
            target="_blank"
            rel="noopener noreferrer"
          >
            <img src="/icons/facebook_logo.png" className="social-media-entity-icon"></img>
          </a>
          <a
          className="social-media-link"
            href={politician.instagram_link}
            target="_blank"
            rel="noopener noreferrer"
          >
            <img src="/icons/instagram_logo.png" className="social-media-entity-icon"></img>
          </a>
          <a
            className="social-media-link"
            href={politician.x_link}
            target="_blank"
            rel="noopener noreferrer"
          >
            <img src="/icons/x_logo.png" className="social-media-entity-icon x"></img>
          </a>
        </div>
      </div>

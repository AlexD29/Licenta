...
const Entities = ({ userId, entityType, apiUrl }) => {
  ...
  const fetchFavorites = async () => {
    try {
      ...
      setFavorites(new Set(response.data.map(item => `${item.entity_id}-${item.entity_type}`)));
    } catch (error) {
  ...
  const fetchHiddenEntities = async () => {
    try {
      const response = await axios.get('http://localhost:8000/api/hidden_entities', {
        params: { user_id: userId }
      });
      setHiddenEntities(new Set(response.data.map(item => `${item.entity_id}-${item.entity_type}`)));
    } catch (error) {
  ...
  const handleFavoriteClick = async (entityId, entityType) => {
    try {
      ...
      const action = favorites.has(`${entityId}-${entityType}`) ? 'remove' : 'add';
  ...
  const handleHideClick = async (entityId, entityType) => {
    try {
      const action = hiddenEntities.has(`${entityId}-${entityType}`) ? 'unhide' : 'hide';
          ...
          return (
            <div key={entityKey} className="entity-card">
              <div className={`entity-details ${entityType}-details`}>
                <div className="entity-details-first-part">
                  ...
                  <div className="button-container">
                    <button 
                      className={`profile-button follow-button ${favorites.has(entityKey) ? 'unfollow-button' : ''}`} 
                      onClick={() => handleFavoriteClick(entity.id, entityType)}
                    >
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                        <path fill="currentColor" d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                      </svg>
                      <span className="tooltip-text">{favorites.has(entityKey) ? 'Unfollow' : 'Add to favourites'}</span>
                    </button>
                    <button className="profile-button hide-button" onClick={() => handleHideClick(entity.id, entityType)}>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
              ...
              <div className="articles-container">
                {entity.articles && entity.articles.map((article, index) => (
                  <Link
                  ...
                  >
                    <div
                      className={`article-card-entity ${article.emotion.toLowerCase()} ${
                        index === entity.articles.length - 1 ? "last-article" : ""
                      }`}
                    >
